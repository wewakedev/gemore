...
window.CartFunctions = {
     ...
     * @param {string} sizeId - Optional size ID
     */
    addToCart: function (productId, quantity = 1, variantId = null, sizeId = null) {
     ...
     * @param {string} sizeId - Optional size ID
     */
    updateQuantity: function (productId, quantity, variantId = null, sizeId = null) {
        if (quantity <= 0) {
            this.removeFromCart(productId, variantId, sizeId);
            return;
        }
     ...
     * @param {string} sizeId - Optional size ID
     */
    removeFromCart: function (productId, variantId = null, sizeId = null) {
        // Convert string "null" to actual null
        if (variantId === "null" || variantId === "") {
            variantId = null;
        }
     ...
     * @param {string} sizeId - Optional size ID
     */
    buyNow: function (productId, quantity = 1, variantId = null, sizeId = null) {
        // First add to cart
        this.addToCart(productId, quantity, variantId, sizeId);
...
window.CartFunctions.updateCartSidebarWithData = function (cartData) {
    ...
    if (cartData.items.length > 0) {
        ...
        itemsContainer.innerHTML = cartData.items
            .map(
                (item) => {
                    const itemPrice = item.variant_size?.price || item.product_variant?.price || item.product.default_variant?.price || item.product.price || 0;
                    const variantName = item.product_variant?.name || '';
                    const sizeName = item.variant_size?.size_display_name || item.variant_size?.size_name || '';
                    const variantId = item.product_variant_id || '';
                    const sizeId = item.variant_size_id || '';
                    
                    // Build the display string for variant and size
                    let variantSizeDisplay = '';
                    if (variantName && sizeName) {
                        variantSizeDisplay = `${variantName} - ${sizeName}`;
                    } else if (variantName) {
                        variantSizeDisplay = variantName;
                    } else if (sizeName) {
                        variantSizeDisplay = sizeName;
                    }
                    ...
                    return `<div class="cart-item-sidebar" data-id="${item.product_id}" data-variant-id="${variantId}" data-size-id="${sizeId}">
                        ...
                        <div class="cart-item-details">
                            ...
                            <div class="cart-item-subtitle">${variantSizeDisplay || item.product.description || ''}</div>
                            <div class="cart-item-price">₹${(itemPrice * item.quantity).toFixed(2)}</div>
                            <div class="quantity-controls">
                                <button class="quantity-btn minus" data-id="${item.product_id}" data-variant-id="${variantId}" data-size-id="${sizeId}">
                                    <i class="fas fa-minus"></i>
                                </button>
                                <span class="quantity-display">${item.quantity}</span>
                                <button class="quantity-btn plus" data-id="${item.product_id}" data-variant-id="${variantId}" data-size-id="${sizeId}">
                                    <i class="fas fa-plus"></i>
                                </button>
                            </div>
                        </div>
                        <button class="remove-item" data-id="${item.product_id}" data-variant-id="${variantId}" data-size-id="${sizeId}">
...
window.CartFunctions.changeQuantity = function (productId, change, variantId = null, sizeId = null) {
    // Convert string "null" to actual null
    if (variantId === "null" || variantId === "") {
        variantId = null;
    }
    ...
    // For sidebar items, we need to find the specific variant and size
    let sidebarSelector = `.cart-item-sidebar[data-id="${productId}"]`;
    if (variantId) {
        sidebarSelector += `[data-variant-id="${variantId}"]`;
    }
    if (sizeId) {
        sidebarSelector += `[data-size-id="${sizeId}"]`;
    }
    const sidebarItem = document.querySelector(sidebarSelector);
    
    if (sidebarItem) {
        // Get variant ID and size ID from the sidebar item if not provided
        const itemVariantId = variantId || sidebarItem.dataset.variantId || null;
        const itemSizeId = sizeId || sidebarItem.dataset.sizeId || null;
        ...
        if (newQty <= 0) {
            this.removeFromCart(productId, itemVariantId, itemSizeId);
            return;
        }
        
        // Make API call to update quantity
        this.updateQuantity(productId, newQty, itemVariantId, itemSizeId);
        return;
    }
    ...
    if (!input) {
        // For cart page, we need to find the specific variant and size
        let cartItemSelector = `[data-product-id="${productId}"]`;
        if (variantId) {
            cartItemSelector += `[data-variant-id="${variantId}"]`;
        }
        if (sizeId) {
            cartItemSelector += `[data-size-id="${sizeId}"]`;
        }
        ...
        if (cartItem) {
            ...
            // Get variant ID and size ID from cart item if not provided
            if (!variantId) {
                variantId = cartItem.dataset.variantId || null;
            }
            if (!sizeId) {
                sizeId = cartItem.dataset.sizeId || null;
            }
        }
    }
    ...
    input.value = newQty;
    this.updateQuantity(productId, newQty, variantId, sizeId);
};
...
(function () {
    ...
    window.CartFunctions.addToCart = function (productId, quantity, variantId, sizeId) {
        // Call original addToCart (which now calls refreshCartState on success)
        originalAdd.call(this, productId, quantity, variantId, sizeId);
...
window.updateQuantity = function (productId, delta, variantId = null, sizeId = null) {
    ...
    if (typeof delta === "number" && (delta === 1 || delta === -1)) {
        ...
        CartFunctions.changeQuantity(productId, delta, variantId, sizeId);
    } else {
        // This is an absolute quantity setting
        CartFunctions.updateQuantity(productId, delta, variantId, sizeId);
    }
};

window.setQuantity = function (productId, quantity, variantId = null, sizeId = null) {
    CartFunctions.updateQuantity(productId, parseInt(quantity), variantId, sizeId);
};

window.removeFromCart = function (productId, variantId = null, sizeId = null) {
    CartFunctions.removeFromCart(productId, variantId, sizeId);
};

window.addToCart = function (productId, quantity = 1, variantId = null, sizeId = null) {
    CartFunctions.addToCart(productId, quantity, variantId, sizeId);
};
    ...
    if (typeof CartFunctions !== "undefined") {
        ...
        document.addEventListener("click", function(e) {
            ...
            if (e.target.matches('.cart-item-sidebar .quantity-btn.minus') || 
                ...
                if (productId) {
                    CartFunctions.changeQuantity(productId, -1, variantId, sizeId);
                }
            }
